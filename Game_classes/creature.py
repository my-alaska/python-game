...
class Creature:
    ...
    def __init__(self, health_points=100, mana_points=100, stamina_points=100, strength=10,
                 power=10, armor=10, agility=10, knowledge=10, magic_resistance=None, magic_attack_type=None):
    ...
    def magic_attack(self, attacked_creature):
        if self.magic_attack_type != None and self.mana_points >= 100:
...
class Magic_attack_type(Enum):
    FIRE = 1
    ICE = 2
    SHOCK = 3


class Magic_resistance():

    def __init__(self, fire_resistance, ice_resistance, shock_resistance):
        self.fire_resistance = fire_resistance
        self.ice_resistance = ice_resistance
        self.shock_resistance = shock_resistance

    def get_magic_resistance(self, magic_attack_type: Magic_attack_type):
        if (magic_attack_type == Magic_attack_type.FIRE):
            return self.fire_resistance
        if (magic_attack_type == Magic_attack_type.ICE):
            return self.ice_resistance
        if (magic_attack_type == Magic_attack_type.SHOCK):
            return self.shock_resistance


class Strategy:

    def __init__(self, moves_loop: list):
